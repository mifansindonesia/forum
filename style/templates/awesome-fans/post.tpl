		...
		<div class="messageMeta ToggleTriggerAnchor">
			...
			<div class="publicControls">
				...
				<xen:if is="{$post.canLike}">
					<a href="{xen:link posts/like, $post}" class="LikeLink item control {xen:if $post.like_date, unlike, like}" data-container="#likes-post-{$post.post_id}"><span></span><span class="LikeLabel"><i class="fa fa-thumbs-o-up faa-vertical animated"></i> {xen:if $post.like_date, {xen:phrase unlike}, {xen:phrase like}}</span></a>
				</xen:if>
				<xen:if is="{$canReply}">
					...
					<a href="{xen:link threads/reply, $thread, 'quote={$post.post_id}'}"
						...
						title="{xen:phrase reply_quoting_this_message}"><span></span><i class="fa fa-comments-o faa-shake animated"></i> {xen:phrase reply}</a>
				</xen:if>
				</xen:hook>
			</div>
		</div>
