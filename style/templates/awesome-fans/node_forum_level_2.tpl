...
<li class="node forum level_{$level} {xen:if '{$level} == 1 AND !{$renderedChildren}', 'groupNoChildren'} node_{$forum.node_id}">
	...
	<div class="nodeInfo forumNodeInfo primaryContent {xen:if $forum.hasNew, 'unread'}">
		...
		<span class="nodeIcon" title="{xen:if $forum.hasNew, '{xen:phrase unread_messages}', ''}">@fa_nodeicon</span>
		...
		<div class="nodeLastPost secondaryContent">
			...
			<xen:elseif is="{$forum.lastPost.date}" />
				<span class="lastThreadTitle"><span>{xen:phrase latest}:</span> <a class="Tooltip" href="{xen:link posts, $forum.lastPost}" title="{$forum.lastPost.title}">{$forum.lastPost.title}</a></span>
		...
		<div class="nodeControls">
			<a href="{xen:link forums/index.rss, $forum}" class="tinyIcon Tooltip" title="{xen:phrase rss}"><i class="fa fa-rss faa-wrench animated-hover"></i></a>
		</div>
