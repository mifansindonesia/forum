		...
		<div class="pageContent">
			...
			<dl class="choosers">
				<xen:if is="{$canChangeStyle}">
					...
					<dd><a href="{xen:link 'misc/style', '', 'redirect={$requestPaths.requestUri}'}" class="OverlayTrigger Tooltip" title="{xen:phrase style_chooser}" rel="nofollow"><i class="fa fa-cogs"></i> {$visitorStyle.title}</a></dd>
				</xen:if>
				<xen:if is="{$canChangeLanguage}">
					<dt>{xen:phrase language}</dt>
					<dd><a href="{xen:link 'misc/language', '', 'redirect={$requestPaths.requestUri}'}" class="OverlayTrigger Tooltip" title="{xen:phrase language_chooser}" rel="nofollow"><i class="fa fa-language"></i> {$visitorLanguage.title}</a></dd>
				</xen:if>
			</dl>
			</xen:if>
			...
			<xen:hook name="footer_links">
				<xen:if is="{$xenOptions.contactUrl.type} === 'default'">
					<li><a href="{xen:link 'misc/contact'}" class="OverlayTrigger Tooltip" data-overlayOptions="{&quot;fixed&quot;:false}" title="{xen:phrase contact_us}"><i class="fa fa-envelope-o"></i><!-- --></a></li>
				...
				<li><a href="{xen:link help}" class="Tooltip" title="{xen:phrase help}"><i class="fa fa-lightbulb-o"></i></a></li>
				<xen:if is="{$homeLink}"><li><a href="{$homeLink}" class="homeLink Tooltip" title="{xen:phrase home}"><i class="fa fa-home"></i> </a></li></xen:if>
				<li><a href="{$requestPaths.requestUri}#navigation" class="topLink Tooltip" title="{xen:phrase top}"><i class="fa fa-chevron-circle-up"></i></a></li>
				<li><a href="{xen:link forums/-/index.rss}" rel="alternate" target="_blank"
					class="Tooltip" title="{xen:phrase rss}"><i class="fa fa-rss"></i></a></li>
			</xen:hook>
			</ul>
			...
			<xen:if is="{$debugMode}">
				<xen:if hascontent="true">
					...
					<xen:contentcheck>
						...
						-->
						<xen:if is="{$page_time}"><i class="fa fa-check-square-o"></i> {xen:phrase x_seconds, 'time={xen:number $page_time, 4}'}</xen:if>
						<xen:if is="{$memory_usage}"> <i class="fa fa-pie-chart"></i> {xen:phrase x_mb, 'size={xen:number {xen:calc "{$memory_usage} / 1024 / 1024"}, 3}'}</xen:if>
						<xen:if is="{$db_queries}"> <i class="fa fa-bar-chart"></i> {xen:number {$db_queries}}</xen:if>
					</xen:contentcheck>
					</dl>
				</xen:if>
			</xen:if>
